 ...
 describe('PillarDAO', () => {
   ...
   /* create named accounts for contract roles */
     ...
     before(async () => {
         ...
         plrToken.balanceOf.whenCalledWith(owner).returns(ethers.utils.parseEther('50000000'));
         plrToken.balanceOf.whenCalledWith(addr1).returns(ethers.utils.parseEther('10000'));
     ...
     it('deposit() with incorrect amount not allowed', async () => {
        ...
        await expect(tran).to.revertedWith('Invalid staked amount');
     });
     ...
     it('deposit() multiple times not allowed', async () => {
        ...
        await expect(tran).to.revertedWith('User is already a member');
     });
     ...
     it('withdraw() fails when not member', async() => {
        ...
        await expect(tran).to.revertedWith('Insufficient balance to withdraw');
     });

     it('withdraw() fails when tried early', async() => {
        const tran = pillarDAO.withdraw();
        await expect(tran).to.revertedWith('Too early to withdraw');
     });
